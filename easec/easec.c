    ...
    while (pos < len) {
        ...
        if (isalpha((unsigned char)c)) {
            ...
            if (!strcmp(id, "raw") || !strcmp(id, "compile")) {
                ...
                if (peek < len && code[peek] == '[') {
                    peek++;
                    int depth = 1; bool in_str = false; int r_start = peek;
                    ...
                    char* raw_c_code = my_strndup(&code[r_start], peek - r_start);
                    trim_inplace(raw_c_code);
                    add_token(!strcmp(id, "raw") ? TOKEN_RAW : TOKEN_COMPILE, raw_c_code);
                    free(raw_c_code); pos = peek + 1; free(id); continue;
                }
            }
...
void exec_c_code(const char* code, bool build_exe) {
    FILE* f = fopen("temp_easec_raw.c", "w");
    if(f) { fputs(code, f); fclose(f); }
    else { printf("Runtime Error: Could not create temp_easec_raw.c\n"); exit(1); }

#ifdef _WIN32
    const char* out_exe = build_exe ? "CompiledOutput.exe" : "temp_easec_raw.exe";
    const char* run_cmd = "temp_easec_raw.exe";
#else
    const char* out_exe = build_exe ? "CompiledOutput" : "./temp_easec_raw.out";
    const char* run_cmd = "./temp_easec_raw.out";
#endif

    char cmd[1024]; 
    snprintf(cmd, sizeof(cmd), "gcc temp_easec_raw.c -o %s", out_exe);
    
    int res = system(cmd);
    if (res != 0) { 
        printf("Runtime Error: C compilation failed. Ensure 'gcc' is installed and in your PATH.\n"); 
        exit(1); 
    }
    
    if (build_exe) {
        printf("Successfully compiled embedded C code to: %s\n", out_exe);
        remove("temp_easec_raw.c");
    } else {
        system(run_cmd); 
        remove("temp_easec_raw.c"); 
        remove(out_exe);
...
Value eval_expr(ASTNode* expr, Env* env) {
    ...
    if (expr->type == NODE_GET) {
        char buf[1024]; if(fgets(buf, 1024, stdin)) buf[strcspn(buf, "\r\n")] = '\0';
        char* end; long long l = strtoll(buf, &end, 10); if(*end == '\0') return val_int(l);
        double d = strtod(buf, &end); if(*end == '\0') return val_float(d);
        if(!strcasecmp(buf, "true")) return val_bool(true); if(!strcasecmp(buf, "false")) return val_bool(false);
    ...
    if (expr->type == NODE_FILE_READ) {
        ...
        if(f) {
            ...
            Value r = val_string(buf); free(buf); return r;
        }
...
void exec_stmt(ASTNode* stmt, Env* env) {
    ...
    if (stmt->type == NODE_RAW) exec_c_code(stmt->c_code, false);
        ...
        FILE* f = fopen(fname, "w"); if(f) { fputs(cnt, f); fclose(f); }
    }
    else if (stmt->type == NODE_FILE_UPDATE) {
        char* fname = value_to_string(eval_expr(stmt->left, env)); char* cnt = value_to_string(eval_expr(stmt->right, env));
        FILE* f = fopen(fname, "a"); if(f) { fputs(cnt, f); fclose(f); }
    }
    else if (stmt->type == NODE_FILE_DELETE) {
        char* fname = value_to_string(eval_expr(stmt->left, env));
        if (remove(fname) != 0) { printf("Runtime Error: File not found to delete.\n"); exit(1); }
    }
...
int main(int argc, char** argv) {
    if (argc != 2) {
        printf("Usage: easec <filename.easec>\n");
        return 1;
    }
    ...
    char* code = malloc(fsize + 1); fread(code, 1, fsize, f); code[fsize] = 0; fclose(f);
