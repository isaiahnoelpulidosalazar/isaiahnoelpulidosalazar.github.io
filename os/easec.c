...
TokenType get_keyword_type(const char* id) {
    ...
    if (!strcmp(id, "increment")) return TOKEN_INCREMENT; if (!strcmp(id, "decrement")) return TOKEN_DECREMENT; if (!strcmp(id, "array")) return TOKEN_ARRAY; if (!strcmp(id, "dictionary")) return TOKEN_DICTIONARY;
    if (!strcmp(id, "boolean")) return TOKEN_BOOLEAN_TYPE; if (!strcmp(id, "number")) return TOKEN_NUMBER_TYPE; if (!strcmp(id, "decimal")) return TOKEN_DECIMAL_TYPE; if (!strcmp(id, "string")) return TOKEN_STRING_TYPE;
    if (!strcmp(id, "true")) return TOKEN_TRUE; if (!strcmp(id, "false")) return TOKEN_FALSE; if (!strcmp(id, "set")) return TOKEN_SET; if (!strcmp(id, "file")) return TOKEN_FILE;
    if (!strcmp(id, "create")) return TOKEN_CREATE; if (!strcmp(id, "read")) return TOKEN_READ; if (!strcmp(id, "update")) return TOKEN_UPDATE; if (!strcmp(id, "delete")) return TOKEN_DELETE;
...
void tokenize(const char* code) {
    ...
    while (pos < len) {
        ...
        if (c == '<') { if (pos+1 < len && code[pos+1] == '=') { add_token(TOKEN_LESS_EQUAL, "<="); pos+=2; continue; } add_token(TOKEN_LESS, "<"); pos++; continue; }
...
void exec_stmt(ASTNode* stmt, Env* env) {
    ...
    else if (stmt->type == NODE_INCDEC) { Value cur = env_get(env, stmt->name); Value amt = eval_expr(stmt->left, env); double c = cur.type == VAL_INT ? (double)cur.as.i : cur.as.f; double a = amt.type == VAL_INT ? (double)amt.as.i : amt.as.f; double r = stmt->is_increment ? c + a : c - a; env_set(env, stmt->name, cur.type == VAL_INT ? val_int((long long)r) : val_float(r)); }
    else if (stmt->type == NODE_IF) { Value cond = eval_expr(stmt->cond, env); bool b = cond.type == VAL_BOOL ? cond.as.b : (cond.type == VAL_INT ? cond.as.i != 0 : false); if (b) { for(int i=0; i<stmt->body_count; i++) { exec_stmt(stmt->body[i], env); if(is_return || is_break) return; } } else { for(int i=0; i<stmt->false_body_count; i++) { exec_stmt(stmt->false_body[i], env); if(is_return || is_break) return; } } }
    else if (stmt->type == NODE_REPEAT) { if (stmt->is_forever) { while(true) { for(int i=0; i<stmt->body_count; i++) { exec_stmt(stmt->body[i], env); if (is_return) return; if (is_break) { is_break = false; return; } } } } else { long long c = eval_expr(stmt->cond, env).as.i; for(long long x=0; x<c; x++) { for(int i=0; i<stmt->body_count; i++) { exec_stmt(stmt->body[i], env); if (is_return) return; if (is_break) { is_break = false; return; } } } } }
    else if (stmt->type == NODE_ARRAY_DECL) { ValueList* list = new_list(); for(int i=0; i<stmt->arg_count; i++) list_add(list, cast_to_type(eval_expr(stmt->args[i], env), stmt->var_type)); Value v; v.type = VAL_ARRAY; v.as.arr = list; env_define(env, stmt->name, v); }
